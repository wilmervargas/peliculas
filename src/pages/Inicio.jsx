...
const Inicio = () => {
    ...
    useEffect(() => {
        ...
        const getDatos = async () => {
            try {
                const response = await fetch(APIPelTendenciasCine, { signal: controller.signal });
                if (!response.ok) {
                    throw new Error(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {datos.map((item) => (
                    <div
                        key={item.id}
                        className="bg-gray-900 rounded-2xl overflow-hidden shadow-lg border border-gray-800 hover:border-gray-700 hover:-translate-y-1 transition-all duration-300 group flex flex-col"
                    >
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                            <h3 className="font-bold text-lg text-white mb-1 line-clamp-1" title={item.title}>
                                {item.title}
                            </h3>
                            <p className="text-gray-400 text-sm mb-4 capitalize">
                                {new Date(item.release_date).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}
                            </p>
                            ...
                            <div className="mt-auto grid grid-cols-2 gap-3">
                                ...
                                <Link to={`/detalle-cine/${item.id}`} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-2 rounded-xl transition-colors shadow-lg shadow-blue-500/20 text-sm flex items-center justify-center">
                                    Detalle
                                </Link>
                            </div>
                        </div>
                        ...
                        <div className="p-6 sm:p-8 md:w-1/2 flex flex-col justify-center order-2 md:order-2 z-10 bg-gray-900">
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <span className="text-gray-400 text-sm capitalize">
                                    {new Date(selectedPelicula.release_date).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}
                                </span>
                            </div>
